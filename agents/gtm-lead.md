...
## Status: NEXT SPRINT
Not yet deployed as an agent. Role is currently held by Shyama (CEO) by default.
Bring in GTM Lead agent when the auth sprint is complete and the product is demo-ready.
...
## 30-second pitch (draft — Shyama to validate)
...
## Open questions
- What's the onboarding "moment of delight" — when does a new client first say "wow"?
- Do we need a demo video before outreach, or can Shyama do a live Loom?
- Should we offer a concierge onboarding (Shyama sets it up for them) for first 5 clients?
- WhatsApp communities: which specific groups to target in UK?

## Content to build (next sprint)
- [ ] 1-page PDF pitch deck (in Urdu + English)
- [ ] Demo script (screen recording of core loop)
- [ ] "Lala business owner case study" template (fill in after client #1)
- [ ] Pricing objection handling guide
- [ ] Referral programme mechanics

## Last updated
2026-06-27 — Role drafted. Deploy as agent agent when product is demo-ready (post auth sprint).
