...
_First version, written 2026-08-22. Revised 2026-08-23 to add an honest talk-track for the single-WhatsApp-session limitation (see the new note under "Before you start," below) — no change to the demo steps themselves, which were re-confirmed working end-to-end in a full dry-run on 2026-08-23. This walks the real, live, currently-working core loop: WhatsApp message in → AI triage → drafted reply → approve/auto-send → quote → invoice → stock deduction. Every step below was verified live in-browser as of the 2026-08-21/22/23 sessions (see `project_product_picker_and_bugs.md` and `project_2026_08_21_session_handoff.md`). Nothing in this script depends on email/IMAP, Tally, or any feature on the cut list — see "What this demo deliberately does not touch," below._

_Companion documents: `PITCH_DECK.md` (the narrative that leads into this), `FIRST_OUTREACH.md` (who you're demoing to), `MARKET_RESEARCH.md` (sourcing behind any competitive claim you make live), `ICP_AND_OBJECTIONS.md` (fuller objection list)._
...
**Know this before they ask it — the single-WhatsApp-number limitation.** Everything in this demo runs on Shyama's own connected WhatsApp number and tenant, not the prospect's. That's normal and fine — it's how every live product demo works, you're not hiding anything by doing this. But be ready for the natural follow-up question, because a sharp prospect will ask it: *"So if I signed up right now, would this be happening on my own number the same way?"* The honest answer is no, not automatically — there is currently one connected WhatsApp session in the product, and a new signup does not yet get an independent, self-serve connection to their own number. Every founding client's number gets connected by Shyama personally as part of onboarding (this is the same concierge-onboarding offer already in `PITCH_DECK.md` Slide 9, not a separate caveat — say it that way, not as an apology): *"Not automatically today, no — right now I connect every founding client's number myself, personally, as part of getting you set up. That's not a workaround, that's the actual offer — you get me hands-on for the first week instead of a signup link and a help article."* Do not imply, in this demo or in `PITCH_DECK.md` Slide 4, that a prospect can scan a QR code themselves today and be live independently — that's not built yet. See `MARKET_RESEARCH.md` for why "founder does the first connection personally" is a legitimate trust-building move in this specific market, not just a technical stopgap.
