...
## 2026-07-26 — Block 1.12c: bootstrap-safe registration RPC applied

**What changed:** Applied `phase_1_12c_bootstrap_tenant_rpc` (draft committed `2530766`, version `20260726171246`) and a follow-on `phase_1_12c_bootstrap_tenant_rpc_grant_correction` (draft committed `481ca6b`). `public.tenants` has no INSERT policy, so `registerWithAuth`'s direct `tenants`/`user_tenants` inserts were broken at the database layer regardless of client — a genuine bootstrap chicken-and-egg problem, since a brand-new user has no tenant and no `user_tenants` row, so no RLS predicate either insert could ever satisfy on its own. Added `public.bootstrap_tenant(p_name text, p_settings jsonb DEFAULT '{}'::jsonb)`, a `SECURITY DEFINER` RPC (`SET search_path = public, pg_temp`) that atomically creates the `tenants` row and the `user_tenants` owner mapping in one transaction. It derives `user_id` from `auth.uid()` and `owner_email` from `auth.users` server-side, hard-codes `role` to `owner`, and never accepts `user_id`, `owner_email`, `tenant_id`, or `role` from the client. It serializes concurrent calls for the same user via `pg_advisory_xact_lock(hashtext(v_uid::text))`, re-checks `user_tenants` for an existing mapping after acquiring that lock (making the RPC itself idempotent and race-safe, not just the app-layer check), and validates `p_name` via `NULLIF(btrim(p_name), '') IS NULL` (SQLSTATE `22023` on blank/whitespace-only names), inserting the trimmed name rather than the raw parameter. `controllers/auth.js#registerWithAuth` now calls `req.supabase.rpc('bootstrap_tenant', ...)` on both the create and idempotent-read paths — `req.supabase` specifically, since `auth.uid()` inside the RPC resolves from the calling request's JWT, not the shared anon client.

**Grant correction:** the first apply's `REVOKE EXECUTE ... FROM PUBLIC` didn't fully match the reviewed design — Supabase's project-level default privileges had already granted EXECUTE to `anon` and `service_role` directly (not via `PUBLIC`) at `CREATE FUNCTION` time, which a `PUBLIC`-only revoke can't remove. Practical impact was none (the function's own `auth.uid() IS NULL` check already rejected unauthenticated callers before any write), but the grant-level boundary didn't match the design either way. The follow-on migration explicitly revoked EXECUTE from `anon` and `service_role` on `bootstrap_tenant(text, jsonb)`; final grants are `authenticated` + `postgres` only, confirmed both in the catalog and behaviourally (`anon` now gets SQLSTATE `42501` — a real grant-layer rejection — before the function body ever runs).

**Result:** no RLS policy was changed on `tenants` or `user_tenants`, `auth_tenant_id()` was not touched, and neither migration created, read, updated, or deleted a single data row — `auth.users`/`tenants`/`user_tenants` row counts stayed at 1/1/1 throughout both applies.

**Verification:** both applies succeeded; function body confirmed via `pg_get_functiondef` unchanged by the grant correction; `pg_proc.proconfig` confirmed `search_path=public, pg_temp`; `information_schema.routine_privileges` confirmed the final `authenticated`+`postgres`-only grant set; `pg_policies` for `tenants`/`user_tenants` diffed byte-for-byte identical pre/post both migrations; Supabase advisor confirms `bootstrap_tenant` no longer under `anon_security_definer_function_executable`, still (expectedly) under `authenticated_security_definer_function_executable`, no new `rls_policy_always_true` regressions. Gated `tests/bootstrapTenantRpc.migration.test.js` passed 2/2 (3 skipped — catalog-only, confirmed via Supabase MCP instead), `tests/auth.test.js` passed 20/20, full suite `npx vitest run` passed 420/78 skipped/0 failed.

**Status:** applied and verified. Full detail in `DB_AUDIT_REPORT.md` §22. **This does not mean multi-tenant isolation is production-ready or that signup is fully proven end-to-end.** No real frontend signup smoke test has been run yet, and no second tenant has ever been created or proven isolated from the first under real `auth.uid()` resolution. Lane C frontend pages, the dev-fallback policy branches, and `tenants`' unscoped `authenticated_read_tenants` SELECT policy all remain untouched. The project-level default-privilege gap this block's grant correction fixed for `bootstrap_tenant()` is a standing gotcha for every future function created in this project, not resolved at the project-default level. Decision Brain remains not started. Inventory Engine untouched.

---

## 2026-07-26 — Block 1.12b: `auth_tenant_id()` hardened and first `user_tenants` mapping backfilled

**What changed:** Applied migration `phase_1_12b_harden_auth_tenant_id` (draft committed `1efb190`, following Block 1.12a's app-code hygiene pass `3ab4e39`). Re-created `auth_tenant_id()` with a pinned `SET search_path = public, pg_temp`, an explicit `public.user_tenants` qualification, and a deterministic `ORDER BY created_at ASC, id ASC` before `LIMIT 1` — replacing an unqualified, unordered lookup that had returned NULL for every authenticated user in this project's history (`public.user_tenants` had 0 rows). Removed the function's redundant blanket `PUBLIC` EXECUTE grant, replacing it with explicit grants to `anon`, `authenticated`, `service_role`, and `postgres`. `anon`'s EXECUTE was **deliberately kept, not revoked** — several scoped RLS policies still evaluate `auth_tenant_id()` as part of an `OR <dev-fallback-tenant>` expression, and several Lane C frontend pages still query those tables with the raw anon browser client, depending on that branch resolving; Postgres checks EXECUTE at evaluation time regardless of which branch of an OR ultimately matches, so revoking it would have broken all of them. Backfilled one `user_tenants` row, mapping the one existing `auth.users` row to the one existing `tenants` row as `role: owner` — the first time `auth_tenant_id()` can return a non-NULL tenant for a real user anywhere in this project's history.

**Result:** no RLS policy was changed on any table, and no dev-fallback branch was removed — this is a function-hardening and data-backfill change only. Supabase's `function_search_path_mutable` advisory for `auth_tenant_id()` is now closed; `anon_security_definer_function_executable` for the same function remains open by design, not a regression.

**Verification:** apply succeeded; `pg_get_functiondef` and `pg_proc.proconfig` confirmed the pinned search_path, qualified table reference, and `ORDER BY` post-apply; `information_schema.routine_privileges` confirmed `PUBLIC` no longer has EXECUTE while `anon`/`authenticated`/`service_role`/`postgres` all retain it; `user_tenants` now has exactly the one expected backfilled row; `auth.users` and `tenants` row counts unchanged; a full pre/post diff of every RLS policy in the `public` schema was byte-for-byte identical. Gated `tests/authTenantId.migration.test.js` passed 2/2 (3 skipped — those require direct catalog access unavailable via the anon-key REST surface, confirmed instead via Supabase MCP queries), full suite `npx vitest run` passed 417/72 skipped/0 failed with no regressions. No residual cleanup needed — the backfilled row is the intended permanent state.

**Status:** applied and verified. Full detail in `DB_AUDIT_REPORT.md` §21. **This does not mean multi-tenant isolation is production-ready.** Only one tenant exists, and nothing here proves isolation between two distinct tenants under real `auth.uid()` resolution. Remaining next priority: `public.tenants` has no INSERT policy at all, so live signup/registration is currently blocked at the database layer regardless of client — the likely next block (1.12c) is a bootstrap-safe registration path, probably a `SECURITY DEFINER` RPC. Lane C pages and the dev-fallback branches remain untouched and gated on that work. Decision Brain remains not started. Inventory Engine untouched.

---

## 2026-07-18 — Block 1.11: `whatsapp_sessions` legacy permissive RLS policy dropped

**What changed:** Applied migration `phase_1_11_drop_whatsapp_sessions_permissive_policy` (draft committed `9ce4831`) — removed the one legacy permissive RLS policy on `whatsapp_sessions` (`"Allow backend to manage sessions"`, `ALL`/`public`/`USING true`/`WITH CHECK true`), independently flagged by Supabase's security advisor as `rls_policy_always_true`. This is the **final table** from the original 9-table legacy-permissive-policy SHOWSTOPPER (Sections 7/10 of `DB_AUDIT_REPORT.md`). Unlike every other table in this family, a repo-wide search found zero application code that ever inserts, updates, or deletes a `whatsapp_sessions` row — the actual WhatsApp connection (`whatsapp-web.js`/Puppeteer in `server.js`) authenticates via filesystem `LocalAuth`, not this DB table. Only SELECT has a live dependent (`controllers/tenants.js#getTenantStatus`, a cosmetic onboarding-progress check), so only one new scoped policy was added — `whatsapp_sessions_tenant_select` — using an explicit `auth_tenant_id()::text` cast, since this table's `tenant_id` is `varchar(255)`, not `uuid` (no implicit uuid↔varchar comparison exists in Postgres). No INSERT/UPDATE/DELETE policy was added; all three are now fully default-deny by design. The VARCHAR→UUID/FK schema fix (`DB_AUDIT_REPORT.md` §3 item 5 / §7 item 3) was deliberately kept out of scope — the inline cast was sufficient to unblock this RLS cleanup without it, and no migration in this family has ever bundled a structural fix with a policy-only change.

**Result:** `whatsapp_sessions` now keeps exactly one policy — the new scoped SELECT. Policy count stayed at 1, but shifted from an unconditional ALL/`true` grant to a tenant-scoped, SELECT-only one.

**Verification:** apply succeeded; `pg_policies` for `whatsapp_sessions` returned exactly 1 policy after apply (`whatsapp_sessions_tenant_select`, `FOR SELECT`, tenant condition confirmed matching the draft exactly, no INSERT/UPDATE/DELETE policy of any kind); RLS remained enabled/not forced; row count unchanged at 0; Supabase's security advisor no longer lists `rls_policy_always_true` for this table; policy counts on all other checked tables were unchanged. Gated `tests/whatsappSessions.migration.test.js` passed 4/4 (1 optional service-role test skipped, no key configured) — confirming anon INSERT is rejected with no FK-vs-RLS ambiguity (unlike every sibling table, `tenant_id` here carries no foreign key at all), anon SELECT returns `[]` against the empty table, and anon UPDATE/DELETE are both empirically default-deny; `tests/tenants.test.js` (mocked, unaffected) passed 9/9; full suite `npx vitest run` passed 413/66 skipped/0 failed with no regressions. No residual test rows were created (INSERT is fully default-deny), so no cleanup was needed — row count confirmed 0 both before and after the gated run.

**Status:** applied and verified. Full detail in `DB_AUDIT_REPORT.md` §20. **The original 9-table RLS legacy-permissive-policy SHOWSTOPPER count is now 0** — every table in that set has had its legacy permissive policy dropped and replaced (or left default-deny) with a scoped, tenant-conditioned policy. Remaining work is no longer "drop the legacy permissive policy" for any of these 9 tables — it is (1) implementing the real `auth.uid() → tenant_id` mapping, since every surviving scoped policy across all 9 tables still runs on the interim dev-fallback-tenant branch, not final production tenant isolation, and (2) `whatsapp_sessions.tenant_id`'s separate VARCHAR → UUID/FK schema cleanup. Decision Brain remains not started. Inventory Engine untouched.

---

## 2026-07-18 — Block 1.10b: `smart_interactions` legacy permissive RLS policies dropped

**What changed:** Applied migration `phase_1_10b_drop_smart_interactions_permissive_policy` (draft committed `0153fdd`) — removed the four legacy permissive RLS policies on `smart_interactions` (`tenant_interactions_select`, `tenant_interactions_insert`, `tenant_interactions_update`, `tenant_interactions_delete`). Unlike `call_logs`/`segments` (zero-scoped starting points), SELECT and INSERT already had correct scoped siblings and were left unchanged; only UPDATE needed a new scoped policy (`smart_interactions_tenant_update`), added with an explicit `WITH CHECK` — not just `USING` — since `tenant_id` is nullable on this table and its three live UPDATE dependents in `drafts/page.tsx` (Dismiss, Escalate, Save-and-Send) are unauthenticated anon calls with no app-level tenant filter at all. This was preceded by the Block 1.10a dispatch-proxy auth hotfix (commit `4e01380`, already merged), which required auth on `/api/responder/dispatch` and scoped its `smart_interactions` SELECT/UPDATE by `req.tenantId` at the app-code level first. DELETE has no scoped replacement — its only known caller anywhere in the repo is gated `autoReplySweeper` test cleanup, not production code — and is now default-deny by design.

**Result:** `smart_interactions` now keeps exactly three policies — scoped SELECT/INSERT (unchanged) plus the new scoped UPDATE. Policy count went from 6 to 3.

**Verification:** apply succeeded; `pg_policies` for `smart_interactions` returned exactly 3 policies after apply, with the new UPDATE policy's `USING` and `WITH CHECK` confirmed identical and matching the expected tenant condition exactly, and no DELETE policy of any kind; gated `tests/smartInteractions.migration.test.js` passed 2/2, including a dedicated tenant-id escape-hatch assertion proving the explicit `WITH CHECK` actually blocks moving a row to `NULL` or a different tenant; default skip-mode runs of both `smartInteractions` and `autoReplySweeper` migration tests were clean; full suite `npx vitest run` passed 413/60 skipped/0 failed with no regressions; policy counts on all other checked tables (including `whatsapp_sessions`) were unchanged. One marked residual row pair was created (no `SUPABASE_SERVICE_ROLE_KEY` configured) — inspected, then cleaned up via the privileged Supabase MCP SQL path (child `smart_interactions` row deleted before its parent `smart_leads` row, matching the FK's lack of `ON DELETE CASCADE`) and confirmed — both tables are back to their pre-test row counts.

**Status:** applied and verified. Full detail in `DB_AUDIT_REPORT.md` §19. Remaining Block 1 RLS SHOWSTOPPER table count reduced from 2 to 1: `whatsapp_sessions` is the only table left.

---

## 2026-07-17 — Block 1.9: `segments` legacy permissive RLS policies dropped

**What changed:** Applied migration `phase_1_9_drop_segments_permissive_policy` (draft committed `a61a73c`) — removed the four legacy permissive RLS policies on `segments` (`tenant_segments_select`, `tenant_segments_insert`, `tenant_segments_update`, `tenant_segments_delete`). Like `call_logs` (Block 1.8), `segments` had zero scoped sibling policies before this migration — but harder: three commands (SELECT, INSERT, UPDATE) all have live frontend anon-client dependents (the segments list view, the "Save Segment" create form, and the "Run Campaign" button's `last_run_at` update), so all three needed new scoped policies added, not just one. The UPDATE policy matters especially here — the frontend's Run Campaign update filters only by `id`, not `tenant_id`, so the scoped UPDATE policy is the sole tenant boundary on that call path. The segments-watch Realtime subscription also depends on the SELECT policy (Realtime does not bypass RLS). DELETE has no app dependent anywhere and is now default-deny by design. `segment_runs`, an adjacent table written by the same Run Campaign flow, carries the identical legacy-only pattern but is out of scope for this block and was not touched. All three new scoped policies still run on the interim dev-fallback-tenant policy branch (`tenant_id = auth_tenant_id() OR tenant_id = <dev-fallback>`), not the final `auth.uid()`-based production tenant mapping.

**Result:** `segments` now keeps exactly three policies — scoped SELECT/INSERT/UPDATE. Policy count went from 4 to 3.

**Verification:** apply succeeded; `pg_policies` for `segments` returned exactly 3 policies after apply (`segments_tenant_select`/`_insert`/`_update`, each single-command, tenant condition confirmed exact, no DELETE policy); gated `tests/segments.migration.test.js` passed 2/2 — confirming anon dev-fallback INSERT/SELECT/UPDATE all work (UPDATE tested filtering only by `id`, matching the real frontend shape), anon DELETE is empirically default-deny (no error, zero rows affected, row still visible), and a nonexistent-tenant INSERT is rejected; `tests/segments.test.js` passed 13/13 (unaffected pure helpers); full suite `npx vitest run` passed 413/57 skipped/0 failed; policy counts on the other checked tables were unchanged. One marked residual test row was created (no `SUPABASE_SERVICE_ROLE_KEY` configured), cleaned up via the privileged Supabase MCP SQL path and confirmed — `segments` is back to 0 rows, matching its pre-migration state.

**Status:** applied and verified. Full detail in `DB_AUDIT_REPORT.md` §18. Remaining Block 1 RLS SHOWSTOPPER table count reduced from 3 to 2: `smart_interactions`, `whatsapp_sessions`. Decision Brain remains gated/not started. Inventory Engine untouched.
