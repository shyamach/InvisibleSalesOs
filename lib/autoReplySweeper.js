...
export async function sweepScheduledReplies(
  ...
  if (error) {
    ...
    logSystemEvent({ category: 'auto_reply', severity: 'error', message: error.message, source: 'lib/autoReplySweeper.js#sweepScheduledReplies' });
 ...
 *   client FACTORY, not a static client. This runs on an indefinite 60s
 *   interval; a Phase D system-JWT client's token expires after 5 minutes
 *   (lib/supabase.js#mintSystemToken), so a client minted once at startup
 *   and reused forever would silently start failing every tick with "JWT
 *   expired" after the first 5 minutes of uptime (a real bug found live,
 *   2026-08-20 — the client used to be passed in pre-built). Calling
 *   getSupabase() fresh on every tick keeps the token always current.
 * @param {Object}        [opts]
 * @param {Function|null} [opts.whatsappSender] — async (to, text) => void; wwebjs client.sendMessage
 * @param {string}        [opts.tenantId] — interim single-tenant scoping (Block 1.7a-2); defaults to DEFAULT_TENANT_ID.
 */
export function startAutoReplySweeper(getSupabase, { whatsappSender, tenantId = DEFAULT_TENANT_ID } = {}) {
  console.log('[AutoReplySweeper]: Started — sweeping scheduled replies every 60s');
  const dispatch = makeDispatch(whatsappSender);

  setInterval(() => {
    sweepScheduledReplies(getSupabase(), { dispatch, tenantId }).catch((err) => {
      console.error('💥 [AutoReplySweeper]: Unexpected error —', err.message);
      logSystemEvent({ category: 'auto_reply', severity: 'error', message: err.message, source: 'lib/autoReplySweeper.js#startAutoReplySweeper' });
    });
  }, SWEEP_INTERVAL_MS);
}
