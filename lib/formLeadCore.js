 ...
 * @param {Function} args.deps.linkLeadContact   — async (leadId, contactId) => void
 * @param {string|null} [args.deps.secret]
 * @param {string} [args.deps.defaultTenant]
...
export async function processFormLead({ body = {}, headers = {}, ip = 'unknown', deps }) {
  const {
    ...
    linkLeadContact,
    secret = null,
    defaultTenant = FALLBACK_TENANT_ID,
  } = deps;
  ...
  if (engineResult?.leadId && contact?.id) {
    try {
      await linkLeadContact(engineResult.leadId, contact.id);
    } catch (err) {
