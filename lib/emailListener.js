  ...
  try {
    ...
    try {
      ...
      if (batchUids.length > 0) {
        ...
        // Collect only — no awaited commands inside this loop. imapflow's
        // fetch() returns an async generator that streams the FETCH
        // response off the same connection; issuing another command (the
        // per-message messageFlagsAdd STORE this loop used to await) before
        // that stream is fully drained deadlocks the connection — the
        // server is still mid-response, the client is waiting on a STORE
        // reply that can't arrive yet, and it sits until imapflow's socket
        // inactivity timeout (5min default) fires as an uncaught exception
        // (confirmed live, 2026-08-24: reproducible every time in-process,
        // while the same fetch+flag sequence against a fresh unshared
        // connection completed in under 2s — see DB_AUDIT_REPORT.md's IMAP
        // section). Mark-as-seen now happens once, batched, after the
        ...
        for await (const msg of messages) {
          ...
          // imapflow's envelope address objects already carry a full
          // `.address` field alongside the split `.mailbox`/`.host` — this
          // used to always reconstruct `${mailbox}@${host}` by hand, which
          // silently produced the literal string "undefined@undefined"
          // whenever a sender's envelope left those two subfields empty
          // (confirmed live 2026-08-25: every Google Play receipt in this
          // tenant's inbox did this, 100% reproducible for that sender
          // format). That garbage string then flowed straight through
          // saveInboundInvoice's vendor_email fallback and rendered
          // verbatim on the invoice detail page. Preferring `.address`
          // fixes it at the source; the mailbox@host reconstruction is
          // kept only as a last-resort fallback for envelopes that
          // somehow have neither.
          const from = msg.envelope?.from?.[0];
          const fromAddress = from?.address || (from?.mailbox && from?.host ? `${from.mailbox}@${from.host}` : null);
          const fromStr = from
            ? `${from.name || ''}${fromAddress ? ` <${fromAddress}>` : ''}`.trim() || fromAddress || 'Unknown'
            : 'Unknown';
        ...
        // Mark as read so we don't re-process on next poll — one batched
        // STORE for the whole page, issued only after fetch()'s generator
        // is fully drained.
        //
        // messageFlagsAdd() does NOT throw on a failed STORE — imapflow's
        // store.js catches connection.exec()'s rejection internally, logs
        // a warning, and resolves to `false` (see its own JSDoc: "returns
        // Did the operation succeed or not"). Silently ignoring that
        // return value here was a real, confirmed-live bug (2026-08-25
        // E2E test, UID 681 on a real Gmail inbox): a transient STORE
        // failure left the message unseen on the server while this
        // function still returned it to the caller as successfully
        // fetched, so pollTenant() dispatched it to onEmail() as normal —
        // then the very next poll's search({seen:false}) found the same
        // still-unseen UID and dispatched it a SECOND time, creating two
        // invoice/lead records from one email. Throwing here instead
        // routes a failed mark through pollTenant()'s existing
        // catch/backoff path, so this batch is retried next poll instead
        // of being treated as already-delivered.
        if (seenUids.length > 0) {
          const marked = await client.messageFlagsAdd(seenUids, ['\\Seen'], { uid: true });
          if (!marked) {
            throw new Error(`Failed to mark ${seenUids.length} message(s) as \\Seen (uids: ${seenUids.join(',')}) — will retry next poll`);
          }
        }
      }
    } finally {
